...
(1, 'Ana Silva', 'Rua das Flores, 123', 'Ribeirão Preto', 'SP', '14400-000', '(16) 1234-5678', 'ana.silva@example.com'),
...
-- Inserir novo livro
INSERT INTO Livro (ID_Livro, Titulo, Data_Publicacao, Categoria_Genero) VALUES
(26, 'O Mistério das Sombras', '2023-12-01', 'Suspense');
...
-- Atualizar endereço do membro com ID_Membro = 41
UPDATE Membro
SET Endereco_Rua = 'Avenida das Palmeiras, 456'
WHERE ID_Membro = 41;

-- Atualizar título do livro com ID_Livro = 26
UPDATE Livro
...
--2. Lista todos os livros da biblioteca ordenados por título
SELECT * FROM Livro ORDER BY Titulo;

--3. Lista todos os empréstimos em aberto
SELECT * FROM Emprestimo WHERE Data_Devolucao IS NULL;

--4. Lista todos os membros que residem na cidade de Ribeirão Preto
SELECT * FROM Membro WHERE Endereco_Cidade = 'Ribeirão Preto';

--5. Lista os empréstimos realizados por um membro específico
...
--8. Lista os empréstimos ordenados por data de devolução mais próxima
SELECT * FROM Emprestimo WHERE Data_Devolucao IS NOT NULL ORDER BY Data_Devolucao ASC;

--9. Lista os empréstimos com multa, ordenados pela maior multa
SELECT * FROM Emprestimo WHERE Multa > 0 ORDER BY Multa DESC;

--10. Lista todos os membros da biblioteca em ordem alfabética por nome
